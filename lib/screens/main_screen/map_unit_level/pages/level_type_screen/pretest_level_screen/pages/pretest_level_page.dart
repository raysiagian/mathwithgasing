...
class _PreTestLevelState extends State<PreTestLevel> {
  ...
  try {
    ...
    final response = await http.put(
      Uri.parse('http://127.0.0.1:8000/api/pretest/${widget.pretest.id_pretest}/update-final-score'),
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (context) => FinalScore(score_pretest: totalScore, materi: widget.materi),
        ),
      );
    } else {
  ...
  Future<void> fetchQuestionPretest() async {
    try {
      final response = await http.get(Uri.parse('http://127.0.0.1:8000/api/getQuestionPretest'));
